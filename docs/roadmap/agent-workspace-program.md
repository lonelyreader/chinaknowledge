...
稳定产品合同见 [`Agent Workspace Requirements`](../agent-workspace-requirements.md)。001–004 已完成并归档；当前唯一 implementation active 子级是 [`AGENT-WORKSPACE-005`](checklists/agent-workspace-compatibility-release.md)。005 Gate 2 已补齐 WorkBuddy publication prepare 确认呈现和 Cursor capability 实际调用，没有 commit 或公共状态变化；Preview、客户端和数据库已恢复，独立复审 `PASS`。当前停在 Gate 3 前，provider、migration 或 Production 仍分别授权。
...
| `AGENT-WORKSPACE-005` | active；Gate 2 PASS，Preview restored | WorkBuddy/Cursor 真实兼容、运营保护、恢复与 Production release | 停在 Gate 3 前；provider、migration、真实身份/数据与 Production 分别批准；TRAE 不在范围 |
...
- 首个执行门：Gate 2 客户端证据完整且独立复审 `PASS`；当前停在 Gate 3 前，PASS 不自动批准后续外部状态或产品改动。
