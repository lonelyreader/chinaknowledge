...
稳定产品合同见 [`Agent Workspace Requirements`](../agent-workspace-requirements.md)。001–004 已完成并归档；当前唯一 implementation active 子级是 [`AGENT-WORKSPACE-005`](checklists/agent-workspace-compatibility-release.md)。005 Gate 5 独立复审 `PASS`，当前进入 Gate 6 public enable；真实内容仍不在授权范围。
...
| `AGENT-WORKSPACE-005` | active；Gate 6 public enable | WorkBuddy/Cursor 真实兼容、运营保护、恢复与 Production release | 只读 real-account smoke + cleanup，TRAE 不在范围 |
...
- 首个执行门：Gate 2–5 均独立复审 `PASS`；当前执行 Gate 6 public enable、单账号只读 smoke、精确 cleanup 与 closure。
