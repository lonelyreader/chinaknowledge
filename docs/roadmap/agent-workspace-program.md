...
稳定产品合同见 [`Agent Workspace Requirements`](../agent-workspace-requirements.md)。001–004 已完成并归档；当前唯一 implementation active 子级是 [`AGENT-WORKSPACE-005`](checklists/agent-workspace-compatibility-release.md)。005 Gate 2 独立复审 `PASS`；用户已批准剩余 Gate 3–6，当前从 exact-path WAF、最小观测和恢复演练开始，后续仍按冻结顺序逐门读回。
...
| `AGENT-WORKSPACE-005` | active；Gate 3 in progress | WorkBuddy/Cursor 真实兼容、运营保护、恢复与 Production release | 剩余门已批准；逐门验证和独立复审，TRAE 不在范围 |
...
- 首个执行门：Gate 2 客户端证据完整且独立复审 `PASS`；用户已批准剩余门，当前执行 Gate 3 Operational amendment C。
