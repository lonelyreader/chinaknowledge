...
当前唯一 implementation active checklist 是 [`AGENT-WORKSPACE-005`](agent-workspace-compatibility-release.md)：Gate 2 已补齐 WorkBuddy publication prepare 确认呈现与 Cursor capability 实际调用，没有 commit 或公共状态变化；Preview、客户端和数据库已精确恢复，独立复审 `PASS`、`P0/P1/P2 = 0/0/0`。当前停在 Gate 3 前；TRAE 不在范围，provider、migration、Production 和真实数据仍分别设门。
