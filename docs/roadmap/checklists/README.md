...
当前唯一 implementation active checklist 是 [`AGENT-WORKSPACE-005`](agent-workspace-compatibility-release.md)：phase-release 的 Gate 1 只读预检和最小 adapter 下载修复已通过 Local 验证与独立复审，当前等待 Gate 2 的 Preview deploy、public MCP、fixture 与真实客户端分别授权，再执行 WorkBuddy 真实 OAuth/MCP、Cursor 回归与清理。TRAE 不在当前适配范围；provider、migration、Production 和真实数据仍分别设门。
