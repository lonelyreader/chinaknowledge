...
- [`AGENT-WORKSPACE-010`](agent-admin-safe-operations.md)：Site Article working-copy 与有限 activity（upgraded），Local/终局复审 PASS，release-only。
- [`AGENT-WORKSPACE-011`](agent-workspace-integration-release.md)：一次性虚构三角色真实 MCP、Preview migration/recovery/cleanup 与 Production 交接（upgraded），唯一 active 执行批。
