...
唯一 active 子级为 [`AGENT-WORKSPACE-001`](checklists/agent-workspace-member-foundation.md)：建立远程 MCP、OAuth 连接与 Member read/draft/preview 首个闭环。代码、依赖、schema、migration、Preview、真实账户和 Production 分别受门禁约束。

## Deferred

| 工作项 | 进入条件 |
|---|---|
| Agent Workspace 002–005 候选阶段 | `AGENT-WORKSPACE-001` closure 后完成父级 transition review，再决定保留、拆分、合并、改序或取消 |
