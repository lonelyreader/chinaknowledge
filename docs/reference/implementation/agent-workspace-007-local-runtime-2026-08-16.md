...
- `my_media_list` 和增强后的 `my_articles_list` 使用当前 owner 的显式边界、最大 50 条分页和固定筛选；Media item 精确投影为 `id / alt / url / status / updatedAt`，Article 列表返回本人 EN/ES 配对状态。
...
负例覆盖他人未公开头像、非 Topic taxonomy、非法链接、未知受保护字段、stale revision 并发、幂等同键异参、prepare 后 Person/角色变化、跨 Member translation、重复与并发 translation、paused/revoked/expired/disabled/scope，以及 connection Person 清空或改绑后的 discovery/call 失败关闭。Profile publication confirmation 的签名域、篡改、过期和 Article/Profile 交叉解析由合同测试覆盖。没有写入真实账号、真实 Person、Preview 或 Production。

## 验证

- `npm --prefix apps/web run test:agent` — PASS。
- 专用 scratch `npm run test:agent:live` — PASS。
- 终审定向修复在全新 scratch 重跑 `test:agent:live` — PASS；覆盖 connection Person 清空/改绑和 Media 五字段精确投影。
