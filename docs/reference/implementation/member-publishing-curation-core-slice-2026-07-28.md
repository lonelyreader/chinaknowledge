...
- Article 的 Payload `_status` 始终由 Member publication 归一化，Member 或 Editor 不能单独改 `_status` 让个人文章消失；Person 未公开时 Article publication 会被服务端拒绝。People Spotlight 只从带 Curated contribution 的人物中选取，完整 People 目录仍保留全部公开人物。
