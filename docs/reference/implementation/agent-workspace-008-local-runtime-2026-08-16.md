...
负例覆盖 Member discovery/直调、任意 assignee/status/where 类输入、其他或 paused assignee、跨 taxonomy、未批准 cover、非法来源、未知 comment row、stale revision、同键异参、site-authored save、curated 不完整回滚和受保护字段 schema；审计只保存 hash 与 reference kind，不含正文、完整核对或评论。没有写入真实账户、真实数据、Preview 或 Production。
