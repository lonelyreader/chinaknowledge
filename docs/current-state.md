...
项目已完成并归档 **P1：可运行公共产品切片**、**P1：编辑 CMS 基础**、**P2：Preview release candidate**、[`PROD-LAUNCH-001`](archive/production-launch-readiness.md) 与 [`PUB-CURATION-001`](archive/member-publishing-curation-closure.md)。`https://chinainfact.com` 已正式匿名公开并允许搜索引擎索引。Production 为 33 张表、12 条 migration；成员直接发布、站方同文档策展与聚焦 Editor 作者体验已经上线并通过双重独立复审。
...
- 正式 Vercel Production deployment [`dpl_BXJNsTa28fwmMqcxt9k22VbCJoWb`](https://china-in-fact-2hb3qkhqv-lonelyreader-c40e168c.vercel.app) 为 `READY / target: production / iad1`，使用 `cms + blob + indexable=true`。它先以受保护候选态完成真实 Home、英西 Article、Person、语言跳转、后台登录页、robots、sitemap 与 390×844 移动端验证，再 promote 到 `chinainfact.com`；100 条运行日志中 error 和 5xx 均为 0。
