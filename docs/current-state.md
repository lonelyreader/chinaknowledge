...
- [`PEOPLE-COMMUNITY-FRONTEND-001`](roadmap/checklists/people-community-frontend.md) 已完成连接优先实现：Home 先找人与人物列表，People 移除 Spotlight/判词专题开场，Person 先呈现身份、当前工作、能帮什么与公开连接动作；公共导航为 `People / Stories / Guides / Places + Join Discord`。提交 `723d84f` 的分阶段 Production 候选 `dpl_FKLCUWhd59G28btAn6wmpnCTW1r8` 已只读回读 Production 的 11 位公开真实人物，桌面/移动 EN/ES、搜索与连接路径通过；候选受 SSO 保护且未切换正式域名。
- [`SITE-TOKEN-SYSTEM-RETHEME-001`](roadmap/checklists/site-token-system-retheme.md) 已在同一 Figma 核心文件建立 119 个 Primitive、Semantic、Typography 与 Dimension 变量、18 个桌面/移动文字样式、3 个效果样式、7 类核心组件及连接/发现/阅读/工具模板；公共站已采用 Geist、Newsreader、Geist Mono 与暖纸/墨色/朱砂语义 Token，旧冷靛蓝只读基线退出。EN/ES 代表页面已完成本地、技术 Preview 与只读真实数据分阶段 Production 回读；正式域名尚未发布这一候选。
...
- 本地应用位于 `apps/web`。当前视觉批次已完成 fixture、本地响应式和受保护 Preview 技术验证；现有本地 CMS 数据库启动时因重复 `(translation_group, locale)=(acceptance-member-curation, en)` 无法创建 `translationGroup_locale_idx`，本批未改 schema 或本地数据。分阶段 Production 候选 `dpl_FKLCUWhd59G28btAn6wmpnCTW1r8` 只读 Production 数据完成 11 位公开 Person、EN/ES、文章、搜索与移动菜单回读，`/api/health=200`，无水平溢出或 browser error/warn；匿名访问仍受 SSO 保护并保持 `noindex`。
...
- 正式 Vercel Production deployment `dpl_CQkJRqNYFHDPhWE7BsXW54KNFoQi`（commit `ce23ba6`，Agent Workspace 33 tools + Person migration 对应应用）仍绑定 `chinainfact.com`；提交 `723d84f` 的新版候选 `dpl_FKLCUWhd59G28btAn6wmpnCTW1r8` 为 `READY / target: production`，但未 promote、未接正式域名。发布前稳定 deployment `dpl_38Ni7qVCZDLnQ4vpiL1eVDuzjLDG` 保留为代码回滚目标。Project 仍连接 `lonelyreader/chinaknowledge`，root=`apps/web`、production branch=`main`。
