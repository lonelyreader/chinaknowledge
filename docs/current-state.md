...
- Preview 已执行 15 条 CMS migration；AGENT-WORKSPACE-011 临时开放后已恢复原 SSO 并回到 Gateway 默认关闭。虚构 User/Person/taxonomy/Media/master/Article、DCR client、connection、workflow 与 Agent event 已按精确 ID 删除；当前 User/Person/taxonomy/Media/master/Article/workflow event 为 `36/32/10/6/60/152/152`，OAuth client/connection/Agent event 为 `0/0/0`。Production 已完成前后 backup/restore、既有 `20260730_181300`、Gateway-off staged release 和公开启用；当前为 14 条 migration，Gateway 公开开启。WorkBuddy、Cursor 与 Codex 已完成既有真实客户端兼容验收；Codex 用现有 Member 完成三项只读调用、隐藏高角色工具、撤销失败关闭与精确 cleanup。User/Person/Media/Article/workflow 仍为 `2/2/2/3/10`，Agent client/connection/event 为 `0/0/0`，真实内容、账号和角色未改写。
...
`AGENT-WORKSPACE-011` 已在专用 `codex/agent-workspace-011-preview` 与 Draft PR [`#2`](https://github.com/lonelyreader/chinaknowledge/pull/2) 完成统一 Preview 集成：精确 Release SHA `af2ffbf5755c397221660bc658e630f7b75eb0b2` 的 33 tools 部署、Preview 第 15 条 Person migration 独立 apply/down/reapply、虚构 Member/Editor/Super Admin 的真实 DCR/OAuth/MCP、三条代表性闭环和权限负例均 `PASS`。fixture、token、connection、event 与临时 env 已精确清理，计数回到基线；关闭态 deployment 为 `READY / preview`，SSO 已恢复 `302`，Gateway off 为 `404`。一次 phase-release 独立复审与定向复核最终 P0/P1/P2=`0/0/0`；`main` 未 push/merge，Production 未执行。
