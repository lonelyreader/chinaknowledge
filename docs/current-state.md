...
项目已完成并归档 **P1：可运行公共产品切片**、**P1：编辑 CMS 基础** 与 **P2：Preview release candidate**。当前唯一 active checklist 为 [`PROD-LAUNCH-001`](roadmap/checklists/production-launch-readiness.md)；受保护 Preview 与不绑定正式域名的 staged Production 均已 `PASS`，Production 为 29/5，并已建立首位 Super Admin、首张真实头像和首个真实 Person 草稿。当前门禁是首篇真实贡献与内容审核；网站域名、公开内容和索引仍未执行。
...
- Preview 与 Production 均已执行全部 5 条 CMS migration，形成 29 张表。Preview 的纯虚构验收数据为 31 个账户、27 个人物、30 篇文章、4 个媒体记录、3 个地点、1 条人物修订和 130 条 workflow event，其中 24 组固定 `.test` 人物与贡献只用于规模、分页、筛选和轮换验收；Production 已有 1 名 Super Admin、1 张经本人授权的真实头像和 1 个真实 Person 草稿，尚无 Article、Place 或公开页面内容，不得把 Preview 夹具复制为 Production 内容。
...
Active 工作及其授权边界以 [`roadmap/README.md`](roadmap/README.md) 为准。`PROD-LAUNCH-001` 的 Preview schema、虚构编辑流、人物规模、分页/筛选、保护、运行日志与独立复审已经通过；邮件、Newsletter、Discord、最低隐私和 Production 环境代码也已通过。Production 现为 29/5，迁移前后恢复 workflow 与不绑定正式域名的 staged deployment 已通过实现者验收和独立复审；首个真实 Person 草稿已建立，下一步是首篇真实贡献与内容审核。
...
- Production Neon 已执行全部 5 条 migration：29 张 `public` 表；首位 Super Admin、1 条已批准真实 Media 和 1 个真实 Person 草稿已建立，articles/places/workflow_events 仍为 0。Person `gexu` 关联本人账户与已批准头像，地点为杭州和墨西哥 Mérida，身份为 Educator and entrepreneur，简介为 A coherentist，语言为英语和西班牙语；因尚无公开 Article，状态保持 Draft。Production Blob 保存头像原图与 card 版本。run [`30351414680`](https://github.com/lonelyreader/chinaknowledge/actions/runs/30351414680) 完成 dump、R2 读回、隔离恢复和 29/5/5/8 断言，users/people/articles/media/workflow_events 为 1/1/0/1/0，并抽样核对媒体 SHA。
