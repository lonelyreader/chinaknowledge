...
`AGENT-WORKSPACE-010` 已完成 Local 实现与工作项验证，等待独立终局复审：本地工具总数增至 33，Super Admin 可从合格且权利已清的中文母稿读取 Body V2，以母稿 content hash 创建同组唯一 EN/ES Site Article 私有 draft，并在 revision、幂等、母稿/Article/引用行锁下保存完整站方 working copy；已公开 Article 的普通保存只形成 pending version，既有逐篇或批次 release 才会推广全部站方字段。`admin_recent_activity` 保持空输入最新 20 条兼容，同时增加最大 50 条分页、首屏 `asOf` 与四项固定筛选。Editor/Member、降权、重复 locale、过期 hash、伪造身份、跨维度引用、未批准媒体、stale revision、并发创建和任意查询均失败关闭；无 schema/migration、UI、通用 CRUD 或账户管理改动。Preview、`main` push 与 Production 尚未执行。
